...
PROJECT_REPO ?= github.com/jfillman/$(PROJECT_NAME)
...
export TERRAFORM_PROVIDER_VERSION ?= 0.19.32
export TERRAFORM_PROVIDER_DOWNLOAD_NAME ?= terraform-provider-infisical
# Infisical is not distributed via releases.hashicorp.com (that's HashiCorp-owned
# providers only) - it ships its own GitHub Releases, goreleaser-built like most
# third-party providers.
export TERRAFORM_PROVIDER_DOWNLOAD_URL_PREFIX ?= https://github.com/Infisical/terraform-provider-infisical/releases/download/v$(TERRAFORM_PROVIDER_VERSION)
# Confirmed by downloading and inspecting the real v0.19.32 darwin_arm64 zip - the
# inner binary has no terraform-protocol-version suffix (unlike e.g. the null
# provider's "_x5"), just "_v<version>".
export TERRAFORM_NATIVE_PROVIDER_BINARY ?= terraform-provider-infisical_v0.19.32
