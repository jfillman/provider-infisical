...
import (
	...
	clusterv1beta1 "github.com/jfillman/provider-infisical/apis/cluster/v1beta1"
	namespacedv1beta1 "github.com/jfillman/provider-infisical/apis/namespaced/v1beta1"
)
...
func TerraformSetupBuilder(version, providerSource, providerVersion string) terraform.SetupFn {
	return func(ctx context.Context, client client.Client, mg resource.Managed) (terraform.Setup, error) {
		...
		//
		// Matches the current hand-rolled operator's model: one shared,
		// instance-admin-scoped credential used for every call this provider
		// makes. In terraform-provider-infisical's own (non-deprecated) terms
		// that's a single admin-scoped machine identity authenticated via
		// Universal Auth - client_id/client_secret at the top level and the
		// bare "service_token" field are both explicitly marked DEPRECATED in
		// the provider's own docs. Org scoping happens per-resource (each
		// resource's own org_id-shaped argument), not here.
